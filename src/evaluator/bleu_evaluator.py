...
def eval_bleu_score(qa_pairs: list[tuple[list[str], str]]) -> float:
    ...
    Args:
        qa_pairs (list[tuple[list[str], str]]): A list of tuples where each tuple contains a list of candidate answers and a reference answer.
    ...
    for references, candidate in qa_pairs:
        result = bleu.compute(predictions=[candidate], references=references)
